...
class App {
    ...
    spiral() {
        ...
        Draw.line(x,y,x+3,y+3,3*x%220,3*_theta%220,3*y%220)
        Draw.setPixel(x,y,20-x,x*y,y)
    }
    ...
    update() {
        _count = _count + 1
        if (_count > 15000){
            System.print(["FREE_MEM",Data.read("FREE_MEM"),"CPU_TEMP",Data.readf("CPU_TEMP").truncate])
            _count = 0
            
            _demo_loops = _demo_loops - 1
            
            if (_demo_loops == 0 &&  Data.read("DEMO_MODE") == 1) App.restartVM("example_4")
        }
    }
    render() {
