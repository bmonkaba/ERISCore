...
class App {
    ...
    spiral() {
        var x = 0
        var y = 0
        _theta = _theta + 0.08 - ((_theta/8400)* (_theta/8400))
        if (_theta > 840.0) { 
            _theta = 0.0
        }
        ...
        var j = Data.read("AM_LOOP_TIME")
        if(j < 4000){
            Draw.line(x,y,__w,__h,(_count)%220,0,(_count * _theta)%220)
        }else{
            Draw.line(x,y,__w,__h,(500000/j)%40,0,(_theta/offset/35)%40)
        }
        Draw.setPixel(x,y,(j)%220,(_theta/3)%220,y%220)
    ...
    update() {
        ...
        if (_count > 5000){
            ...
            if (_demo_loops == 0 &&  Data.read("DEMO_MODE") == 1) App.restartVM("example_16")
        }
    }
    render() {
        ...
        //System.print("**")
        var seconds = System.clock.truncate.toString
        Draw.setTextColor(180,180,0)
        Draw.setCursor(__x + 1,__y + 1)
        Draw.print(seconds)
        Draw.setCursor(__x - 1,__y - 1)
        Draw.print(seconds)
        Draw.setCursor(__x + 1,__y - 1)
