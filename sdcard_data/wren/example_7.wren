...
class App {
    ...
    spiral() {
        var x = 0
        var y = 0
        _theta = _theta + 0.07111
        if (_theta > 120.0) { 
            _theta = 0.0
        }
        x = (((_theta/Num.pi).cos * _theta/3.0) + __w/2).truncate
        y = (((_theta/Num.pi).sin * _theta/3.0) + __h/2).truncate
        var offset = (Data.read("INPUT_PEAK")/32000.0) * -30
        Draw.line(x,y,x+offset,y+offset,(x*20)%220,(y*20)%220,(_count)%220)
        Draw.setPixel(x,y,(20-x)%220,(_count)%220,_count%220)
    }
    ...
    updateRT() {
        ...
        if (_count > 5000){
            ...
            if (_demo_loops == 0 &&  Data.read("DEMO_MODE") == 1) App.restartVM("example_8")
        }
    }
    update() {
        __w = __w + __dw
        ...
        if (__w < 24) __dw = __dw * -1
        if (__h < 24) __dh = __dh * -1
        App.setWidgetDimension(__w, __h)
        ...
        __x = __x + _dx
        __y = __y + _dy
        if (__x < 0 - __w/2) _dx = _dx * -1
        if (__y < 0 - __h/2) _dy = _dy * -1
        if (__x > 320 - __w/2) _dx = _dx * -1
        if (__y > 240 - __h/2) _dy = _dy * -1
    }
    onFocus() {
        var a = "test"
    }
