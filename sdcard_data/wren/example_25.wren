...
class App {
    ...
    //
    createPerlin() {
        var blocks = 2.pow(_max_iterations-_iteration)
        ...
        for (row in 0...(blocks)){
            ...
            var b = ((200-r)/3)%220 //App.random(120)
            var g = ((r + b) + App.random(110))%220 //App.random(120)
            ...
            for (y in from_y...(to_y)) {
                for (x in (from_x)...(to_x)) {
                    ...
                    var ar = ((_max_iterations/2-_iteration) * r + (_iteration-1)*(pc >> 8))/(_max_iterations * 1)
                    var ag = ((_max_iterations/2-_iteration) * g + (_iteration-1)*((pc >> 3) & 252))/(_max_iterations * 1)
                    var ab = ((_max_iterations/2-_iteration) * b + (_iteration-1)*((pc << 3)  & 255))/(_max_iterations * 1)
    ...
    update() {
        ...
        if (_count > 5000){
            ...
            if (_demo_loops == 0 &&  Data.read("DEMO_MODE") == 1) App.restartVM("example_26")
        }
    }
    render() {
