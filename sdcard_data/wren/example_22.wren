...
class App {
    ...
    spiral() {
        _theta = _theta + _theta/190
        if (_theta > 420.0 + __w + __h) { 
            _theta = 50.0
            Draw.fill(30,0,30)
        }
        var x = ((_theta/Num.pi).cos * _theta/3.0) + __w/2
        var y = ((_theta/Num.pi).sin * _theta/3.0) + __h/2
        //Draw.line(x,y,x+offset,y+offset,(x)%220,0%220,(0)%220)
        Draw.setPixel(x,y,(220 - (x+__x))%220,(_count)%220,((__y+y+80)/2)%220)
    }
    ...
    updateRT() {
        ...
        if (_count > 2900){
            ...
            if (_demo_loops == 0 &&  Data.read("DEMO_MODE") == 1) App.restartVM("example_23")
